...
export function SelectUi(props: SelectUiParams): JSX.Element {
  ...
  const handleClick = (event: any) => {
    ...
    if (
      target.id !== `${props.name}-result` &&
      target.id !== `${props.name}-options`
    ) {
  ...
  return (
    <div className={cx("header-select__language", "text-sm rounded-xl")}>
      <span
        id={`${props.name}-result`}
        className={cx("header-select__language--result", "rounded-xl")}
        onClick={() => setOpenOption(true)}
      >
        {props.valueDefault}
        <FontAwesomeIcon
          icon={faChevronDown}
          className={cx("header-select__icon")}
        />
      </span>
      {openOption && (
        <div
          className={cx("header-select__language--option", "rounded-xl")}
